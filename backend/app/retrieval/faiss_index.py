...
class FaissIndexBuilder:
    def __init__(self, embeddings_path: str = None, index_path: str = None):
        self.embeddings_path = embeddings_path or os.path.abspath(
            os.path.join(
                os.path.dirname(__file__), "../../../artifacts/candidate_embeddings.npy"
            )
        )
    ...
    def build_and_save(self):
        ...
        print(
            f"Loaded embeddings of shape {embeddings.shape} in {time.time() - start_time:.2f} seconds."
        )

        num_candidates, dimension = embeddings.shape

        print(
            f"Initializing FAISS IndexFlatIP (Flat Inner Product) index for dimension {dimension}..."
        )
