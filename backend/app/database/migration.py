...
class DatabaseMigrator:
    ...
    def migrate(self, jsonl_path: str, batch_size: int = 1000):
        ...
        insert_fts_sql = "INSERT INTO candidate_fts (candidate_id, document_text) VALUES (?, ?)"
        ...
        for cand in self.stream_jsonl(jsonl_path):
            ...
            if len(candidates_batch) >= batch_size:
                self._execute_batch(
                    insert_cand_sql, candidates_batch,
                    insert_skill_sql, skills_batch,
                    insert_history_sql, history_batch,
                    insert_cert_sql, certs_batch,
                    insert_fts_sql, fts_batch
                )
        ...
        print(f"Data migration finished. Ingested total: {total_migrated} profiles in {time.time() - start_time:.2f} seconds.")

    def _execute_batch(
        self,
        cand_sql, cand_list,
        skill_sql, skill_list,
        hist_sql, hist_list,
        cert_sql, cert_list,
        fts_sql, fts_list
    ):
