...
class CandidateFeatureVector(BaseModel):
    cross_encoder: float = Field(
        ..., description="Normalized semantic Cross-Encoder score (Sigmoid applied)"
    )
    experience: float = Field(
        ..., description="Gaussian experience matching score [0, 1]"
    )
    ...
    engagement: float = Field(
        ..., description="Normalized candidate activity and engagement index [0, 1]"
    )
