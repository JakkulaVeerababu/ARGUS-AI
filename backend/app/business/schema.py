...
class ExperienceRequirement(BaseModel):
    ...
    max_years: Optional[float] = Field(
        default=None, description="Maximum preferred years of experience"
    )


class JDRequirementsSchema(BaseModel):
    must_have_skills: List[str] = Field(
        default_factory=list, description="Mandatory technical skills"
    )
    nice_to_have_skills: List[str] = Field(
        default_factory=list, description="Nice-to-have or optional skills"
    )
    experience: ExperienceRequirement = Field(
        default_factory=ExperienceRequirement,
        description="Target experience parameters",
    )
    locations: List[str] = Field(
        default_factory=list, description="Target cities or regions"
    )
    titles: List[str] = Field(default_factory=list, description="Target job titles")
    companies: List[str] = Field(
        default_factory=list, description="Preferred companies"
    )
