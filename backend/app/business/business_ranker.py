...
def generate_feature_explanation(
    ...
    if location_score >= 0.95:
        reason += (
            " Located directly in or regionally adjacent to the target hiring area."
        )
...
def rank_candidate(
    ...
    if isinstance(parsed_jd, dict):
        ...
        min_y = (
            exp_req.get("min_years")
            if isinstance(exp_req, dict)
            else exp_req.min_years if hasattr(exp_req, "min_years") else None
        )
        max_y = (
            exp_req.get("max_years")
            if isinstance(exp_req, dict)
            else exp_req.max_years if hasattr(exp_req, "max_years") else None
        )
    ...
    comp_score = calculate_company_score(
        candidate_past_companies=past_companies, preferred_companies=jd_req.companies
    )
