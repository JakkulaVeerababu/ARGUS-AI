...
class LocalSubmissionValidator:
    ...
    def validate(csv_path: str) -> bool:
        ...
        official_validator_path = os.path.abspath(os.path.join(base_dir, "data/validate_submission.py"))
        
        if not os.path.exists(official_validator_path):
            # Try nested folder path
            official_validator_path = os.path.abspath(
                os.path.join(base_dir, "[PUB] India_runs_data_and_ai_challenge/[PUB] India_runs_data_and_ai_challenge/India_runs_data_and_ai_challenge/validate_submission.py")
            )
            
        if not os.path.exists(official_validator_path):
            print(f"Error: Official validator not found at expected paths.")
            return False
            
        print(f"Loading official validator from: {official_validator_path}")
        
        # Load module dynamically
        spec = importlib.util.spec_from_file_location("validate_submission", official_validator_path)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print("Usage: python -m backend.app.reasoning.submission_validator <path_to_submission.csv>")
