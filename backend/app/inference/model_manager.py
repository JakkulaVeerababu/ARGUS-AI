    ...
    def get_model_and_tokenizer(self, model_name_or_dir: str):
        ...
        with self.load_lock:
            if model_name_or_dir in self.sessions:
                return (
                    self.sessions[model_name_or_dir],
                    self.tokenizers[model_name_or_dir],
                )
            ...
            if not os.path.exists(onnx_path):
                ...
                print(
                    f"ONNX model file not found at {onnx_path}. Triggering conversion..."
                )
                export_models()

            if not os.path.exists(onnx_path):
                raise FileNotFoundError(
                    f"Failed to load/convert ONNX model at: {onnx_path}"
                )

            print(
                f"Loading ONNX InferenceSession for '{model_name_or_dir}' from: {onnx_path}"
            )
