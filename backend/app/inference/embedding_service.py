...
class EmbeddingService:
    ...
    def _ensure_loaded(self):
        if self.session is None or self.tokenizer is None:
            self.session, self.tokenizer = self.manager.get_model_and_tokenizer(self.model_name)

    def encode(self, texts: Union[str, List[str]], batch_size: int = 64, normalize_embeddings: bool = True) -> np.ndarray:
        """
        ...
        for i in range(0, len(texts), batch_size):
            ...
            for input_meta in self.session.get_inputs():
                ...
                if name in encoded_inputs:
                    ...
                    onnx_inputs[name] = np.zeros_like(encoded_inputs["input_ids"], dtype=np.int64)
