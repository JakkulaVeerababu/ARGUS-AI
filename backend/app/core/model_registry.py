...
class ModelRegistry:
    ...
    def initialize(self):
        ...
        with self._lock:
            ...
            logger.info(
                "ModelRegistry: Initializing ModelManager & warming up ONNX models..."
            )
            ...
            logger.info(
                "ModelRegistry: Initializing HybridSearcher (loads FAISS and lexical mappings)..."
            )
            ...
            self._reranking_manager = CrossEncoderRerankingManager(
                candidates_file=settings.CANDIDATES_PATH
            )
            self._reranking_manager.reranker.load_model()

            logger.info(
                "ModelRegistry: Warmup completed successfully. All models loaded into memory."
            )
    ...
    def sqlite_manager(self) -> SQLiteManager:
        if self._sqlite_manager is None:
            raise RuntimeError(
                "ModelRegistry is not initialized. Call initialize() first."
            )
