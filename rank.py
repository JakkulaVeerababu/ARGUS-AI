...
def run_ranking_pipeline(candidates_path: str, jd_path: str, output_csv: str):
    ...
    retrieved_candidates = hybrid_searcher.search(
        query_text, top_k_branch=1000, final_top_n=300, k_rrf=60
    )
    ...
    reranked_candidates = reranking_manager.rerank(
        query=query_text, candidate_ids=candidate_ids, top_n=200
    )
    ...
    scored_candidates = business_scorer.score_candidates(
        ce_results=reranked_candidates, top_n=200
    )
    ...
    for c_record in scored_candidates:
        ...
        if risk_res["risk_score"] > 0.5:
            print(
                f"  [Filtered] High-Risk {cid} - Score: {risk_res['risk_score']:.2f} - Reasons: {risk_res['reasons']}"
            )
            high_risk_count += 1
            continue
    ...
    print(
        f"Gatekeeper complete. Filtered {honeypot_count} honeypots and {high_risk_count} high-risk candidates."
    )
    ...
    if len(safe_candidates) < 100:
        print(
            f"Warning: Only found {len(safe_candidates)} safe candidates. Filling with remainder."
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Argus AI Candidate Discovery and Ranking CLI"
    )
    parser.add_argument(
        "--candidates",
        type=str,
