...
def test_experience_engine_gaussian():
    ...
    s_under = calculate_experience_score(3.0, min_years=5.0, max_years=9.0, sigma_low=1.5, sigma_high=3.5)
    s_over = calculate_experience_score(11.0, min_years=5.0, max_years=9.0, sigma_low=1.5, sigma_high=3.5)
...
def test_location_engine():
    ...
    assert calculate_location_score("Pune, India", targets, willing_to_relocate=True) == 0.85
    
    # Mismatch
    assert calculate_location_score("Pune, India", targets, willing_to_relocate=False) == 0.50
...
def test_business_ranker_integration():
    candidate = {
        ...
        "skills": [
            {"name": "Python"}, {"name": "PyTorch"}, {"name": "FAISS"}
        ],
        "career_history": [
            {"company": "Google"}
        ],
