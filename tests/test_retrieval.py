...
def test_bm25_tokenization():
    tokens = BM25Searcher.tokenize("Python developer, 5 years experience! Django expert.")
    # Should lowercase, split on word boundaries, remove punctuation
    assert tokens == ["python", "developer", "5", "years", "experience", "django", "expert"]
...
def test_rrf_fusion_logic():
    ...
    bm25_ranks = {cid: rank for rank, (cid, _) in enumerate([("A", 10.0), ("B", 8.0)], 1)}
    faiss_ranks = {cid: rank for rank, (cid, _) in enumerate([("B", 0.95), ("A", 0.90)], 1)}
